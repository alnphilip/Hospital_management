...
export default function DoctorPrescriptions() {
    ...
    async function loadData() {
        ...
        if (doctor) {
            ...
            const { data: rxs } = await supabase.from("prescriptions").select("*").eq("doctor_id", doctor.id).order("created_at", { ascending: false });
            setPrescriptions(rxs || []);

            const { data: apts } = await supabase.from("appointments").select("id, appointment_date, patient_id").eq("doctor_id", doctor.id).eq("status", "assigned");
    ...
    async function handleCreate(e: React.FormEvent) {
        ...
        else { toast.success("Prescription created!"); setShowModal(false); loadData(); }
        setSubmitting(false);
    }
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                    <div>
                        ...
                        <select value={form.appointment_id} onChange={(e) => setForm({ ...form, appointment_id: e.target.value })} required className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/50">
                            ...
                            {appointments.map((a) => <option key={a.id} value={a.id}>{a.appointment_date} — {a.id.slice(0, 8)}</option>)}
                        </select>
                    </div>
                    <div>
